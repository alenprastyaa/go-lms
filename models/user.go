...
type User struct {
	...
	FailedLoginLockedUntil    *time.Time `gorm:"column:failed_login_locked_until" json:"failed_login_locked_until"`
	SchoolID                  *uint      `gorm:"column:school_id" json:"school_id"`
	ClassID                   *uint      `gorm:"column:class_id" json:"class_id"`
	InitialPasswordCiphertext *string    `gorm:"column:initial_password_ciphertext" json:"-"`
