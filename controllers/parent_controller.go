...
func (a *AppContext) issueParentSession(c *fiber.Ctx, parent *models.User) error {
	...
	if parent.SchoolID != nil {
		_ = a.DB.Select("name", "logo_url", "inventory_module_enabled", "attendance_module_enabled", "official_exam_module_enabled", "koperasi_module_enabled", "private_chat_module_enabled", "teaching_module_ai_enabled").Where("id = ?", *parent.SchoolID).First(&school).Error
