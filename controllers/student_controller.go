...
func (a *AppContext) EditStudent(c *fiber.Ctx) error {
	...
	a.DB.Raw(`SELECT id, full_name, username, role, class_id, parent_email, phone_number FROM users WHERE id = ?`, id).Scan(&current)
	if current.ID == 0 {
	...
	`, coalesceStrPtr(body.FullName, current.FullName), username, role, classID, coalesceStrPtr(body.ParentEmail, current.ParentEmail), coalesceStrPtr(body.PhoneNumber, current.PhoneNumber), schoolID, id).Scan(&row)
...
func (a *AppContext) RegisterStudentByAdmin(c *fiber.Ctx) error {
	...
	`, body.FullName, body.Username, string(hash), body.Role, schoolID, body.ClassID, body.ParentEmail, body.PhoneNumber).Scan(&row)
